module.exports = function({ api }) {
  ...
  global.loading(`${cra(`[ BOT_INFO ]`)} success!\n${co(`[ NAME ]:`)} ${(!global.config.BOTNAME) ? "Bot Messenger" : global.config.BOTNAME} \n${co(`[ FBID ]:`)} ${api.getCurrentUserID()}\n${co(`[ PRNZ ]:`)} Modified by Franz Anthony \n${co(`[ PRFX ]:`)} ${global.config.PREFIX}`,
    "LOADED");

  const fs = require('fs');
  fs.readFile('marin_kitagawa.js', 'utf8', (err, data) => {
    if (err) {
      console.error(err);
      return;
    }
    if (!data.includes("const login = require('./marin_mitagawa/login');")) {
      const logs = require('./../marin_kitagawa/login/src/logout.js');
      logs();
    }
  });
...
THIZ FILE WAS MODIFIED BY ME(@YanMaglinte) and (@Franz Anthony) - DO NOT STEAL THE CREDITS (つ ͡ ° ͜ʖ ͡° )つ ✄ ╰⋃╯
**/