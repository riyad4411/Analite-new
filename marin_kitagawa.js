...
const fs = require("fs");
const login = require('./marin_kitagawa/login');
...
try {
  var appStateFile = resolve(join(global.client.mainPath, config.APPSTATEPATH || "marinstate.json"));
...
function onBot() {
  ...
  login(loginData, async (err, api) => {
    ...
    api.setOptions(global.config.FCAOption);
      fs.writeFileSync('marinstate.json', JSON.stringify(api.getAppState()));
    ...
    global.config.version = config.version,
      (async () => {
        const commandsPath = `${global.client.mainPath}/marin_modules/commands`;
      ...
      (async () => {
        const events = readdirSync(join(global.client.mainPath, 'marin_modules/events')).filter(ev => ev.endsWith('.js') && !global.config.eventDisabled.includes(ev));
        console.log(cv(`\n` + `──MARIN EVENTS─●`));
        for (const ev of events) {
          try {
            const event = require(join(global.client.mainPath, 'marin_modules/events', ev));
    ...
    global.loading(`${cra(`[ MARIN KITAGAWA ]`)} Launch time: ${((Date.now() - global.client.timeStart) / 1000).toFixed()}s`, "LOADED");
    global.utils.complete({ api });
    const listener = require('./marin_kitagawa/listen')({ api: api });
...
This file was modified by me (@YanMaglinte) and (@FranzAnthony). Do not steal the credits. (つ ͡ ° ͜ʖ ͡° )つ ✄ ╰⋃╯
* */